...
namespace rmos_detector_l
{
    ...
    {
    public:
        MixDetectorNode(const rclcpp::NodeOptions &options) : BaseDetectorNode("mix_detector_l", options)
        {
            RCLCPP_INFO(this->get_logger(), "Starthing mix_detector_l node");
            

            //subscriber
            this->image_sub_ = std::make_shared<image_transport::Subscriber>(image_transport::create_subscription(
                    this, "/image_raw_l", std::bind(&MixDetectorNode::imageCallBack, this, std::placeholders::_1),
                    "raw",
