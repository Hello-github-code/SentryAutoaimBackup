...
namespace rmos_processer_r {
    ProcesserNode::ProcesserNode(const rclcpp::NodeOptions &options) : Node("processer_r", options) {
        RCLCPP_INFO(this->get_logger(), "Start processer_node_r");
        controler_ = std::make_shared<processer::Controler_r>();
        ...
        this->prosser_server_ =this->create_service<sentry_interfaces::srv::AimTarget>("AimTarget_r",std::bind(&ProcesserNode::sentrycallback,this,std::placeholders::_1,std::placeholders::_2));
        ...
        armors_sub_.subscribe(this, "/rmos_detector/armors_r", rmw_qos_profile_sensor_data);
        ...
        this->quaternion_sub_ = this->create_subscription<rmos_interfaces::msg::QuaternionTime>("/imu_quaternion_r", rclcpp::SensorDataQoS(),
        ...
        this->bs_sub_ = this->create_subscription<rmos_interfaces::msg::BulletSpeed>("/bs_info_r", rclcpp::SensorDataQoS(),
        ...
        this->autoaim_state_sub_ = this->create_subscription<rmos_interfaces::msg::AutoaimState>("/autoaim_state_r", rclcpp::SensorDataQoS(),
                                                                                                 std::bind(&ProcesserNode::autoaimStateCallBack, this, std::placeholders::_1),
                                                                                                 autoaim_state_sub_option);


        // set publisher
        this->target_pub_ = this->create_publisher<rmos_interfaces::msg::Target>("/target_r", rclcpp::SensorDataQoS());
        this->sentry_target_pub_ = this->create_publisher<sentry_interfaces::msg::FollowTarget>("/follow_target_r",1);
                this->aim_pub_ = this->create_publisher<rmos_interfaces::msg::Aimpoint>("/aim_r", rclcpp::SensorDataQoS());

        this->yaw_mark_pub_ = this->create_publisher<std_msgs::msg::Float32>("/yawr_marke",rclcpp::SensorDataQoS());

        armor_marker_.ns = "armors_cube";
        armor_marker_.header.frame_id = "world_r";
        ...
        text_marker_.header.frame_id = "world_r";
        ...
        this->detect_marker_pub_ =
                this->create_publisher<visualization_msgs::msg::MarkerArray>("/detect/marker_r", 10);
        this->process_marker_pub_ =
                this->create_publisher<visualization_msgs::msg::MarkerArray>("/process/marker_r", 10);
    ...
    void ProcesserNode::armorsCallBack(const rmos_interfaces::msg::Armors::SharedPtr armors_msg)
    {
        ...
        if (quaternion_buf_.size() > 0) {
            ...
            for (auto &armor: armors_msg->armors) {
                ...
                try {
                    guess_pose.pose = tf2_buffer_->transform(gs, "camera_r").pose;
                }                
                catch (const tf2::LookupException &ex) {
                    RCLCPP_ERROR(get_logger(), "Error while transforming %s", ex.what());
                    return;
                ...
                // if(attack_id>=0)
                // if(((armor_pitch/3.1415926*180)<-10||(armor_pitch/3.1415926*180)>25)&&controler_->tracker_[attack_id].rotate_target_state[1]>5)
                // {
                //     continue;
                //      switch (armor.type)
                // {
                // case false:
                // cv::solvePnP(small_armor, point2D, camera_matrix_, dist_coeffs_, correct_rvec, correct_tvec, false, cv::SOLVEPNP_ITERATIVE);
                // break;
                // case true:
                // cv::solvePnP(big_armor, point2D, camera_matrix_, dist_coeffs_, correct_rvec, correct_tvec, false, cv::SOLVEPNP_ITERATIVE);
                // break;
                // default:
                // cv::solvePnP(big_armor, point2D, camera_matrix_, dist_coeffs_, correct_rvec, correct_tvec, false, cv::SOLVEPNP_ITERATIVE);
                // break;
                ...
                //  cv::Rodrigues(correct_rvec, rotation_matrix);
                //  tf2::Matrix3x3 tf2_rotation_matrix_(
                //     rotation_matrix.at<double>(0, 0), rotation_matrix.at<double>(0, 1),
                //     rotation_matrix.at<double>(0, 2), rotation_matrix.at<double>(1, 0),
                //     rotation_matrix.at<double>(1, 1), rotation_matrix.at<double>(1, 2),
                //     rotation_matrix.at<double>(2, 0), rotation_matrix.at<double>(2, 1),
            ...
            rclcpp::Clock steady_clock_{RCL_STEADY_TIME};
              ...
              for(int i=0;i<14;i++)
               ...
               sentry_transform = tf2_buffer_->lookupTransform ("world_r", "camera_r",armors_msg->header.stamp);
           ...
           // std::cout<<"gl"<<attack_id<<'\n';
            ...
            for(int i=0;i<14;i++)
            {
                //std::cout<<'c'<<count_id<<' '<<"p"<<(int)prioritys[i]<<'\n';
                if(attack_id==(int)prioritys[i])
                break;
                count_id++;

            }
            // std::cout<<"stateeeeeee=="<<controler_->tracker_[3].tracker_state<<'\n';
          //  std::cout<<"stateeeeeee=="<<controler_->tracker_[attack_id].lost_thres<<"count=="<<controler_->tracker_[attack_id].lost_count_<<'\n';
            ...
            if (move_state != 3) {
                ...
                cv::Point3f p_y_t = controler_->ballistic_solver_.getAngleTimer(aiming_point*1000);
                // if(abs(controler_->tracker_[attack_id].target_state[7])>2.0)
                // {
                //     cv::Point3f gun_point(controler_->tracker_[attack_id].target_state[0],controler_->tracker_[attack_id].target_state[2],aiming_point.z);
                //     p_y_t = controler_->ballistic_solver_.getAngleTimer(gun_point*1000);
                // }
                ...
                //std::cout << "delta yaw::" << move_state << std::endl;
                ...
                std::string oral_frame_ = "camera_r";
                try
                {
                    p2x.pose = tf2_buffer_->transform(p2x, oral_frame_).pose;
                }
                catch (const tf2::LookupException &ex)
                {
                    RCLCPP_ERROR(get_logger(), "Error while transforming %s", ex.what());
                ...
                std::string oral_frame = "camera_r";
                try
                {
                ...
                if(gun_pitch>15)
                is_fire=false;
                target_msg.suggest_fire = is_fire;//isfire
...
RCLCPP_COMPONENTS_REGISTER_NODE(rmos_processer_r::ProcesserNode)
