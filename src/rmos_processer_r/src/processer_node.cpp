...
namespace rmos_processer_r {
    ProcesserNode::ProcesserNode(const rclcpp::NodeOptions &options) : Node("processer_r", options)
    {
        ...
        this->prosser_server_ =this->create_service<sentry_interfaces::srv::AimTarget>("AimTarget_r", 
        ...
        this->bs_sub_ = this->create_subscription<rm2_referee_msgs::msg::ShootData>("/rm2_referee/shoot_data", rclcpp::SensorDataQoS(),
        ...
        this->yaw_mark_pub_ = this->create_publisher<std_msgs::msg::Float32>("/yawr_marker", rclcpp::SensorDataQoS());
    ...
    void ProcesserNode::armorsCallBack(const rmos_interfaces::msg::Armors::SharedPtr armors_msg)
    {
        ...
        for (auto &armor: armors_msg->armors) {
            ...
            try {
                guess_pose.pose = tf2_buffer_->transform(gs, "RCam_Link").pose;
            }
            catch (const tf2::LookupException &ex) {
                RCLCPP_ERROR(get_logger(), "Error while transforming %s", ex.what());
                return;
        ...
        sentry_transform = tf2_buffer_->lookupTransform("Gimbal_Link", "RCam_Link", armors_msg->header.stamp);
        ...
        if (move_state != 3)
        {
            ...
            if (if_center)
            {
                ...
                std::string oral_frame_ = "RCam_Link";
                try {
            ...
            std::string oral_frame = "RCam_Link";
            try {
