...
typedef void(ORT_API_CALL *OrtLoggingFunction)(void *param,
...
typedef enum OrtMemType {
  ...
  OrtMemTypeCPU =
    OrtMemTypeCPUOutput,  ///< Temporary CPU accessible memory allocated by non-CPU execution provider, i.e. CUDA_PINNED
...
typedef struct OrtTensorRTProviderOptions {
  ...
  int
    trt_int8_use_native_calibration_table;  // use native TensorRT generated calibration table. Default 0 = false, nonzero = true
  ...
  int
    trt_force_sequential_engine_build;  // force building TensorRT engine sequentially. Default 0 = false, nonzero = true
...
struct OrtApiBase {
  ...
  const char *(ORT_API_CALL *GetVersionString)(void)
    NO_EXCEPTION;  ///< Returns a null terminated string of the version of the Onnxruntime library (eg: "1.8.1")
};
...
struct OrtApi {
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(SessionEndProfiling,
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(CreateAllocator,
                  _In_ const OrtSession *session,
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(RunWithBinding,
                  _Inout_ OrtSession *session,
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(CreateAndRegisterAllocator,
                  _Inout_ OrtEnv *env,
                  _In_ const OrtMemoryInfo *mem_info,
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(CreateArenaCfg,
                  _In_ size_t max_mem,
                  int arena_extend_strategy,
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(GetTensorMemoryInfo,
                  _In_ const OrtValue *value,
...
#ifdef __cplusplus
  OrtApi(const OrtApi &) =
    delete;  // Prevent users from accidentally copying the API structure, it should always be passed as a pointer
#endif
};
