#ifndef RM_V7_C_POST_PEOCESSER_HPP_
#define RM_V7_C_POST_PEOCESSER_HPP_
...
public:
    ...
    void process(DetectResultList & output) override 
    {
        /*
            [1 * 15120 * 28]
            x y w h conf [class conf *9] x1 y1 s1 x2 y2 s2 x3 y3 s3 x4 y4 s4 [color conf]
        */     
        ...
        for (size_t i = 0; i < tensor_shape_[1]*tensor_shape_[2]; i+=tensor_shape_[2])
        {
            ...
            // double min_point_score = 1;
            // double max_point_score = 0;
            double ave_point_score = 0;

            // double color_point_score = 0;

            for (int j = 1; j <= key_points_num_; j++) {
                // if (p_tensor_[i + 6 + 3*j] < min_point_score) {
                //     min_point_score = p_tensor_[i + 6 + 3*j];
                // }
                // else if (p_tensor_[i + 6 + 3*j] > max_point_score) {
                //     max_point_score = p_tensor_[i + 6 + 3*j];
                // }
                ave_point_score += p_tensor_[i + 15 + 3*j];
            }
            ave_point_score /= key_points_num_;

            // color_point_score = p_tensor_[i + 14 + 3*3 +1];

//             std::cout << "ave_point_score is ____" << ave_point_score << std::endl;
//             std::cout << "color_point_score is ____" << color_point_score << std::endl;

            if (ave_point_score > 0.33  &&  ave_point_score < 0.66) {
                max_id += 22;  // N
            }
            else if (ave_point_score > 0.66) {
                max_id += 11;  //Red
            }
            ...
            DetectResult pre_result = {
                ...
                .points = {
                    // x y w h conf [class conf 11] x1 y1 s1 x2 y2 s2 x3 y3 s3 x4 y4 s4 color
                        cv::Point2d(
                                (p_tensor_[i + 16] - remap_params_.ox) / remap_params_.scale,
                                (p_tensor_[i + 17] - remap_params_.oy) / remap_params_.scale
                            ),
                        cv::Point2d(
                                (p_tensor_[i + 19] - remap_params_.ox) / remap_params_.scale,
                                (p_tensor_[i + 20] - remap_params_.oy) / remap_params_.scale
                            ),
                        cv::Point2d(
                                (p_tensor_[i + 22] - remap_params_.ox) / remap_params_.scale,
                                (p_tensor_[i + 23] - remap_params_.oy) / remap_params_.scale
                            ),
                        cv::Point2d(
                                (p_tensor_[i + 25] - remap_params_.ox) / remap_params_.scale,
                                (p_tensor_[i + 26] - remap_params_.oy) / remap_params_.scale
                            )
                    },
                .id = max_id,
                .conf = max_conf
...
//             // << result.points[4].x << ", " << result.points[4].y << "]["
//             << std::endl;
//         }
