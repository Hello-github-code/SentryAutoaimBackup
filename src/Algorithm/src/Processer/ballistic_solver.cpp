...
namespace processer
{
    ...
    cv::Point3f BallisticSolver::getAngleTimel(cv::Point3f position)
    {
        ...
        double t_actual = 0.0;
        double y_temp = position.z / 1000.0+0.05;
        double y = y_temp;
        double x = sqrt(position.x * position.x + position.y * position.y) / 1000.0;
         //std::cout<<"bsl="<<this->bullet_speed_<<'\n';
        for (int i = 0; i < 40; i++) {
    ...
    cv::Point3f BallisticSolver::getAngleTimer(cv::Point3f position)
    {
        ...
        double dy, angle, y_actual;
        double t_actual = 0.0;
        double y_temp = position.z / 1000.0+0.024;
        double y = y_temp;
        double x = sqrt(position.x * position.x + position.y * position.y) / 1000.0;
            //   std::cout<<"bsr="<<this->bullet_speed_<<'\n';
    ...
    void BallisticSolver::setBS_coeff_l(cv::Point3f position)
    {
        ...
        if(distance>3.5)bs_coeff*=1.025;
