...
namespace camera
{
    ...
    bool DahengCam::open(GX_OPEN_PARAM *pOpenParam)
    {
        if (is_open_)
        {
            return true;
    ...
    bool DahengCam::reset_parameters(const std::map<CamParamType, int> &param_map ,GX_OPEN_PARAM *pOpenParam)
    {
        this->close();
        for (auto it = param_map.begin(); it != param_map.end(); it++)
            this->set_parameter(it->first, it->second);
        return this->open(pOpenParam);
    }
    ...
    bool DahengCam::init_sdk(GX_OPEN_PARAM *pOpenParam)
    {
        GX_STATUS status;
        ...
        //status = GXGetAllDeviceBaseInfo(pDeviceInfo_,pBufferSize_);



    
       //std::cout<<pBufferSize_<<'\n';

       //std::cout<<(*pDeviceInfo_).szSN<<'\n';
            //      char sn[32]= "NF0190070004";

          //pOpenParam->pszContent=sn;
    
        //std::cout<<*(pOpenParam->pszContent)<<'\n';

        //status = GXSetString (&device_,GX_STRING_DEVICE_SERIAL_NUMBER,pOpenParam->pszContent);



        /*TODO*/
        // Open device by sn
        //status = GXOpenDeviceByIndex(i, &device_); //  open the  device 1-l  2-r
            status = GXOpenDevice(pOpenParam,&device_);
        if (status != GX_STATUS_SUCCESS)
        {
            std::cout<<"omggg//////////////////////////////////////////"<<'\n';
            GXCloseLib();
