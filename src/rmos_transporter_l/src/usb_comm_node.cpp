...
{
    UsbCommNode::UsbCommNode(const rclcpp::NodeOptions &options) : CommNode("usb_comm_l", options)
    {
        ...
        this->target_sub_ = this->create_subscription< geometry_msgs::msg::QuaternionStamped>
                ("/head_left/quaternion", rclcpp::SensorDataQoS(),
                 std::bind(&UsbCommNode::targetCallBack, this, std::placeholders::_1),
                 target_sub_options);
        this->target_sub_r_ = this->create_subscription<geometry_msgs::msg::QuaternionStamped>
                ("/head_right/quaternion", rclcpp::SensorDataQoS(),
        ...
        //     target_sub_options);
        // std::cout << "pidl " << interface_usb_pid_ << '\n';

        // this->quaternion_pub_ = this->create_publisher<rmos_interfaces::msg::QuaternionTime>("/imu_quaternion_l", rclcpp::SensorDataQoS());

        // this->receive_timer_ = this->create_wall_timer(1ms, std::bind(&UsbCommNode::recevieCallBack, this));
        // 可尝试定时器会不会出问题 再用线程
        // this->recevie_thread_l = std::thread(&UsbCommNode::recevieCallBack, this);

        quaternion_time_msg_.quaternion_stamped.header.frame_id = std::string("Imu_l");
    ...
    void UsbCommNode::targetCallBack(const geometry_msgs::msg::QuaternionStamped::SharedPtr quaternion_time_msg_)
    {
        // std::cout << "FFFFFFF" << '\n';
        ...
        t.transform.rotation.x = quaternion_time_msg_->quaternion.x;
        t.transform.rotation.y = quaternion_time_msg_->quaternion.y;
        t.transform.rotation.z = quaternion_time_msg_->quaternion.z;
        t.transform.rotation.w = quaternion_time_msg_->quaternion.w;
        tf_publisher_->sendTransform(t);
 
        //std::cout << "l_perception_send:" << (int16_t)(perception_yaw_);
        ...
        //     transporter_->write((unsigned char *)&send_package_, sizeof(transporter::RMOSSendPackage));
        // }
        //std::cout << (int32_t)send_package_.AimbotState << std::endl;
        // this->target2state(target, &(send_package_.AimbotState));
    ...
    void UsbCommNode::othertargetstateCallBack(const geometry_msgs::msg::QuaternionStamped::SharedPtr quaternion_time_msg_)
    {   
        geometry_msgs::msg::TransformStamped t;
        t.header.stamp =  this->now();
        t.header.frame_id = "Gimbal_Link"; //注意坐标系
...
RCLCPP_COMPONENTS_REGISTER_NODE(rmos_transporter_l::UsbCommNode)
