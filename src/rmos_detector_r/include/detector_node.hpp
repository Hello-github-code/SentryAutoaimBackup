...
namespace rmos_detector_r
{
    ...
    {
    public:
        BasicDetectorNode(const rclcpp::NodeOptions &options) : BaseDetectorNode("basic_detector_r", options)
        {
            RCLCPP_INFO(this->get_logger(), "Starthing basic_detector_r node");

            //subscriber
            this->image_sub_ = std::make_shared<image_transport::Subscriber>(image_transport::create_subscription(
                    this, "/image_raw_r", std::bind(&BasicDetectorNode::imageCallBack, this, std::placeholders::_1),
                    "raw",
