...
namespace rmos_processer_l {
    ProcesserNode::ProcesserNode(const rclcpp::NodeOptions &options) : Node("processer_l", options)
    {
        ...
        this->prosser_server_ = this->create_service<sentry_interfaces::srv::AimTarget>("AimTarget_l",
        ...
        this->yaw_mark_pub_ = this->create_publisher<std_msgs::msg::Float32>("/yawr_markel", rclcpp::SensorDataQoS());    // 调试用
    ...
    void ProcesserNode::armorsCallBack(const rmos_interfaces::msg::Armors::SharedPtr armors_msg)
    {
        ...
        // std::cout << "armorsCallBack Start Success!!!" << std::endl;
        ...
        // if (quaternion_buf_.size() > 0) {
            ...
            sentry_transform = tf2_buffer_->lookupTransform("Gimbal_Link", "LCam_Link", armors_msg->header.stamp);
            ...
            for (auto &armor: armors_msg->armors) {
                ...
                //////////// 得到 armor 在 "Gimbal_Link" 坐标系下的坐标, 用于后面 guess_pose 的计算 //////////
                ...
                //////////// 计算得到在 "LCam_Link" 坐标系下的坐标 guess_pose //////////
                ...
                try {
                    guess_pose.pose = tf2_buffer_->transform(gs, "LCam_Link").pose;
                }                
                catch (const tf2::LookupException &ex) {
                    RCLCPP_ERROR(get_logger(), "Error while transforming %s", ex.what());
                    return;
                ...
                //////////// 根据 guess_pose 计算出对应的姿态 correct_rvec 和位置 correct_tvec, 作为 Pnp 的初始值 //////////
                ...
                //////////// 使用 Pnp 计算出正确的姿态 correct_rvec 和位置 correct_tvec //////////
                ...
                //////////// 再通过姿态 correct_rvec 和位置 correct_tvec 计算出对应的在 "Gimbal_Link" 坐标系下的 armor.pose //////////
                ...
                tf2::fromMsg(armor.pose.orientation, tf_armor_q);
                tf2::Matrix3x3(tf_armor_q).getRPY(armor_roll, armor_pitch, armor_yaw);
                double armor_line_yaw=atan(armor.pose.position.y/armor.pose.position.x);
                armor_yaw=armor_line_yaw+armor.yaw;
               
                if ( abs(armor_pitch / M_PI * 180) > 20 && abs(armor_roll / M_PI * 180) > 5 )
                    continue;

                // std::cout << "armor_yaw = " << armor_yaw / M_PI * 180 << "   armor_pitch = " << armor_pitch / M_PI * 180 << "   armor_roll = " << armor_roll / M_PI * 180 << std::endl;
            ...
            if (move_state != 3) {
                ...
                std::string oral_frame = "LCam_Link";
                try {
