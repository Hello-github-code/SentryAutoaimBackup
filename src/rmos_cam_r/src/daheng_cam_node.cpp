...
{
    DahengCamNode::DahengCamNode(const rclcpp::NodeOptions &options) : CamNode("daheng_camera_r", options)
    {
        // cam dev
        ...
        cv::FileStorage fs("./rmos_bringup/configure/camera_right/daheng_camera_r.xml", cv::FileStorage::READ);
        if(!fs.isOpened())
        {
        ...
        auto yaml_path = "file://" + pkg_path + "/configure/camera_right/daheng_cam_info_r.yaml";
        if (!cam_info_manager_->loadCameraInfo(yaml_path))
        {
            RCLCPP_WARN(this->get_logger(), "Load Camera_R Info Fail!");
        }
        else
        {
        ...
        camera_info_pub_ = create_publisher<sensor_msgs::msg::CameraInfo>("/daheng_camera_info_r", 10);
